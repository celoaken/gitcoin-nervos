...
export function App() {
	...
	console.log('generated deposit address = ' + depositAddress.addressString);
    ...
    async function updateSudtBalance(proxyContract, account) {
	const addressTranslator = new AddressTranslator();
	const balance = await proxyContract.methods.balanceOf(addressTranslator.ethAddressToGodwokenShortAddress(account)).call({
	   from: account
	});
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            const _sudtProxyContractAddress = '0x869d30cDa85F8f6421cCEb0935a239c6B5480b07';

	    const proxyContract = new _web3.eth.Contract(ERC20JSON.abi, _sudtProxyContractAddress);

            updateSudtBalance(proxyContract, _accounts[0]);
        })();
    });
    ...
    return (
	    ...
	    <br />
            <br />
            You can use <a href={'https://force-bridge-test.ckbapp.dev/bridge/Ethereum/Nervos?xchain-asset=0x0000000000000000000000000000000000000000&recipient=' + layer2DepositAddress} target="_blank">Force Bridge</a> to make deposit to the layer 2, make sure that the receiver address is same as above
	    <br />
            <button disabled={!layer2DepositAddress} onClick={(e) => {
		   e.preventDefault();
                   window.open('https://force-bridge-test.ckbapp.dev/bridge/Ethereum/Nervos?xchain-asset=0x0000000000000000000000000000000000000000&recipient=' + layer2DepositAddress, '_blank');
		}
	    }>
            Deposit through Force Bridge
            </button>
            <br />
            <br />	
            Balance: <b>{balance ? (balance / 10n ** 8n).toString() : <LoadingIndicator />} ETH</b>
            <br />
            SUDT Balance: <b>{sudtBalance ? sudtBalance.toString() : <LoadingIndicator />} ckETH</b>
            <br />
            <br />
